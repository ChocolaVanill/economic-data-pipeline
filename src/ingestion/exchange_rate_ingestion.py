...
def ingest_exchange_rates_data():
    client = APIClient()
    engine = get_engine()
    ...
    with engine.connect() as conn:
        conn.execute(
            text("""
            INSERT INTO bronze.exchange_rates_raw (api_endpoint, response_status, raw_data, row_count, ingestion_batch_id)
    ...
    logger.info(f"Ingested {len(data)} exchange_rates records with batch_id: {batch_id}")
    return batch_id

...
def ingest_cpi_data():
    ...
    data = client.fetch(ENDPOINTS["cpi_headline"])

    with engine.connect() as conn:
        conn.execute(
            text("""
            INSERT INTO bronze.cpi_raw (api_endpoint, response_status, raw_data, row_count, ingestion_batch_id)
    ...
    logger.info(f"Ingested {len(data)} CPI records with batch_id: {batch_id}")
    return batch_id
